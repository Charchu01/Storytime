...
export default function Overview() {
  ...
  const dbConfigured = data?.supabaseConfigured !== false;

  return (
    <div>
      {!dbConfigured && (
        <div style={{ ...card, border: "1px solid #fef08a", background: "#fefce8", marginBottom: 20 }}>
          <h3 style={{ ...cardTitle, color: "#a16207", margin: "0 0 8px" }}>Supabase Not Configured</h3>
          <p style={{ fontSize: 13, color: "#854d0e", margin: 0 }}>
            The admin dashboard requires Supabase to store analytics data. Ensure SUPABASE_URL and SUPABASE_SERVICE_KEY are set,
            and that the admin migration has been applied (supabase-migration-admin.sql).
          </p>
        </div>
      )}
      {/* Key Metrics */}
